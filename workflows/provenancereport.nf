...
def renderedReportMultiqc(report_name, report_path) {
    def report_name_text = escapeHtml(report_name)
    def report_path_text = escapeHtml(report_path)

    return """
    id: 'nf-core-provenancereport-rendered-report'
    description: 'Rendered Quarto report generated by the pipeline.'
    section_name: 'Rendered Quarto Report'
    plot_type: 'html'
    data: |
      <dl class="dl-horizontal">
        <dt>Report file</dt><dd><a href="../${report_path}"><samp>${report_name_text}</samp></a></dd>
        <dt>Published path</dt><dd><samp>${report_path_text}</samp></dd>
      </dl>
    """.stripIndent().trim()
}
