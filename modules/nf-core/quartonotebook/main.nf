...
process QUARTONOTEBOOK {
    ...
    tuple val("${task.process}"), val("${workflow.containerEngine ?: (task.conda ? 'conda' : 'none')}"), val("${workflow.containerEngine ? task.container ?: 'Not configured' : task.conda ?: 'Not configured'}"), emit: runtime_environment
