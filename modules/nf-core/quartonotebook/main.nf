...
process QUARTONOTEBOOK {
    ...
    tuple val("${task.process}"),
          val("${workflow.containerEngine ?: (task.conda ? 'conda' : 'none')}"),
          val("${workflow.containerEngine ? task.container ?: 'Not configured' : task.conda ?: 'Not configured'}"),
          eval('printf "%s" "${CONDA_PREFIX:-Not configured}"'),
