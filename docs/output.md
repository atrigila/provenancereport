...
- The resolved container reference and container engine used by `QUARTONOTEBOOK`, the active Nextflow profile, `R sessionInfo()` output, and Python version. R and Python are reported as unavailable when they are absent from the report container.
- Pipeline, Nextflow, Quarto, and Papermill versions.
